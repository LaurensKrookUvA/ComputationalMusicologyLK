...
<br>
The outliers negatively affect overmixing. At least on a basic level of mixing. From the DJ industry, songs with a maximum tempo difference of 6 bpm can be mixed. A so-called Master Tempo function on the turntable ensures that the key does not change at that moment. See a helpful explainer [video](https://www.youtube.com/watch?v=azZTKcBZsRw) here. When the tempo difference is greater than 6 bpm, the listener will clearly hear the difference in tempo. A possible solution to compensate for the large bpm difference is to use tempo harmonics and subharmonics. For example, a 120 BPM song can be mixed with the 60 BPM subharmonic. In the following visualization it is examined whether this is used.

### How does the tempo behave throughout the playlist?
```{r}
...
***
The second visualization shows the progression of tempo over the playlist. On the x axis here the songs are numerically represented in playlist order. On the y axis, the tempo is shown. The figures are analyzed by playlist. The second visualization complements the first visualization well. Both figures clearly show that Dance Rising and mint have the most outliers. Looking at the order, the technique of subharmonics (as discussed on the previous page) is probably used once. In the dance rising playlist this can be seen in around track 45 and 46. The tempo difference between track 45 (80 bpm) and track 46 (160 bpm). Unfortunately, in cases of the large tempo difference, this technique is not used more often. We can therefore already draw a first conclusion that in these cases the transitions in terms of tempo are random.
<br>
The best build in tempo can be seen in the technobunker playlist. In this visualization, the pace is clearly building and decreasing. There are few peaks in the visualization, which means that techniques such as subharmonics are not applicable. This is positive for the transitions

### An outlier that makes the transition complicated even for a regular DJ. {data-commentary-width=400}
```{r}
...
### What does a good sounding transition look like?
```{r, echo=FALSE}
...
### How does the key relate to the playlists?
```{r}
...
Energy and Valence {data-navmenu="Important musical elements for analysis" data-icon="ion-android-happy"}
=======================================================
### {.sidebar data-width=400}
-------------------------------------------------------
In the visualizations above, a number of variables are plotted for each playlist. On the x-axis the valence (range 0 to 1), on the y-axis the energy (also a range of 0 to 1), the size of the dots represented the loudness (-60dB to 0 db). Finally, the color distinction distinguishes between major and minor. Since this is an interactive visualization, when hovering over the dots, the name of the specific numbers can be found. The plots are analyzed one by one. 
<br> 
Starting with the Dance Rising playlist, one notices that the energy does not go below 0.46. In this area, no outliers can be found in this playlist. Most of the songs are between the energy range of 0.6-1. The valence distribution is very divided. It varies mainly between 0 and 0.9. The loudness is similar for most songs. Nevertheless, one outlier stands out. This is the song Where do we go by Dimension and Poppy Baskcomb. After listening to this song, there are many high notes in it and the singer sings very high and loud. This explains the outlier compared to the other songs in this playlist. The distribution major/minor is scattered. 
<br> 
The visualization to the right of this concerns the playlist Housework. Here there is no song with an energy lower than 0.5. The valence is very scattered. There are no further outliers to be found in terms of loudness and mode. It is noticeable that the data points are more clipped than the first visualization discussed. 
<br> 
The third visualization (bottom left) concerns the playlist mint. Compared to all other playlists, the data points here are the most scattered. The energy varies over a broader domain. The valence is scattered though most data points are below a range of 0.8. Again, the loudness and mode show no outliers. 
<br> 
Finally, the TechnoBunker playlist is analyzed. Immediately visible is that the data points are very clustered at the top left of the figure. Compared to the other playlists, the valence is much lower and does not exceed 0.7. This can be explained because techno songs often have an industrial dark atmosphere.  In this visualization it is relatively easy to see that there are more songs in major than minor. The loudness shows no outliers. 
In conclusion, we can say that most playlists have high energy and distributed valence with the exception of the techno bunker playlist. The mode and loudness shows no outliers in most cases.

Column
--------------------------------------------------------
### Can valence, energy, loudness and mode give an unexpected addition?
```{r}
en_valplot <- all_data |>                    # Start with awards.
  mutate(
  ...
  facet_wrap(~ playlist_name) +    # Separate charts per playlist.
...
=========================================================
### La Fuente - I Want You, An interesting homogeneity pattern
```{r}
bzt <-
...
### Tom Santa - Rainfall, Checkerboards everywhere
```{r}
bzt <-
...
Clustering {data-navmenu="Important musical elements for analysis" data-icon="ion-ios-keypad" .storyboard}
=========================================================

### Which features are important for improving mixing?
```{r}
data_feature_analysis <-
  bind_rows(
    housewerk |> mutate(playlist = "Housewerk") |> slice_head(n = 20),
    technobunker |> mutate(playlist = "Techno Bunker") |> slice_head(n = 20),
    dance_rising |> mutate(playlist = "Dance Rising") |> slice_head(n = 20),
    mint |> mutate(playlist = "mint") |> slice_head(n = 20)
  ) |> 
  add_audio_analysis()
```

```{r}
indie_feature_analysis <-
  data_feature_analysis |>  # For your portfolio, change this to the name of your corpus.
  mutate(
...
feature_analyis_recipe <-
  recipe(
    ...
    data = indie_feature_analysis           # Use the same name as the previous block.
  ) |>
...
```{r}
feature_analysis_cv <- indie_feature_analysis |> vfold_cv(10)
```
...
feature_analysis_forest <- 
  workflow() |> 
  add_recipe(feature_analyis_recipe) |> 
...
workflow() |> 
  add_recipe(feature_analyis_recipe) |> 
  add_model(feature_analysis_model) |> 
  fit(indie_feature_analysis) |> 
  pluck("fit", "fit", "fit") |>
  ranger::importance() |> 
  enframe() |> 
...
### Improvements by using clustering
```{r}
...
```{r}
housewerk_clust <-
  get_playlist_audio_features("mint", "37i9dQZF1DX4dyzvuaRJ0n") |>
  slice_head(n = 20) |>
  add_audio_analysis() |>
  mutate(
    segments = map2(segments, key, compmus_c_transpose),
...
housewerk_juice <-
  recipe(
    track.name ~
      ...
      c01 + c02 + c03 + c04 + c05 + c06 +
      c07 + c08 + c09 + c10 + c11 + c12,
    data = housewerk_clust
  ) |>
  step_center(all_predictors()) |>
  step_scale(all_predictors()) |> 
  # step_range(all_predictors()) |> 
  prep(housewerk_clust |> mutate(track.name = str_trunc(track.name, 20))) |>
  juice() |>
  column_to_rownames("track.name")
```

```{r}
housewerk_dist <- dist(housewerk_juice, method = "euclidean")
housewerk_dist |> 
...
Conclusion and Discussion{data-icon="ion-clipboard"}
=========================================================
### Conclusion
The conclusion summarizes the results and answers the research question posed in the introduction. 
<br>
The introduction introduced the research question: "Does the spotify automix functionality use the same techniques as a regular DJ?". In this portfolio, visualizations were used to try to answer this question. 
<br>
Starting with the visualizations on tempo. From the visualizations, it can be seen that the Dance Rising and mint playlist contain the most outliers in terms of tempo. This negatively effects the overmixing of the songs. Housework and Technobunker are the same in terms of tempo. The songs are in the same tempo range and few outliers can be found there. In addition, there is a single song that makes the transition tremendously difficult as seen in the last visualization in terms of tempo. In the song Affraid to feel by LF SYTEMS, the tempo chart showed that the song consists of different tempos that does not make the transition to another song easier.
<br>
In the visuals about key, the first insight was generated into how many songs were properly over-mixed based on key. This number was low for all four playlists. Using one example, a good transition was shown and using another example, a bad transition was shown. Using the circle of fifth's, the difference between a good and a bad transition became easily visible. 
<br>
From the energy and valence figure, it could be seen that the playlists are very energetic and the valence is very scattered. From the loudness, few outliers could be found. The biggest finding found in this visualization is that the technobunker playlist consists primarily of songs in major. 
<br>
The Chromograms showed that by eye it is very difficult to tell which chords were used in songs. Therefore, the least data was obtained from this to answer the research question. 
<br>
The similarity matrices were interesting. It was clear from the first figure that La Fuente's song I Want You contained multiple choruses but representation was difficult to see. In contrast, the second visualization showed more insight on novelty. The visualizations showed good insight into repetitions and changes in the song. The visualizations did not have a major impact on answering the research question but showed good insights. 
<br>
The last visualizations in this portfolio regarding the clustering summarize the experiences made in the portfolio. The portfolio showed that the most information regarding the research question could be obtained from the tempo and key components. As mentioned earlier in this conclusion, the transitions made by Spotify's Automix functionality are not yet optimal. By means of clustering a method is shown how feature selection and tree's can be used to adjust the order within the playlist in order to improve the number of transitions.
